...
import { AuxiResumenNotas as ResumenNotas } from "./NotasResumen";
// import type { MateriaResumen } from "./AuxiResumenNotas";
...
export function AuxiliarResumen() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      try {
        ...
        const materias: Materia[] = await apiFetch.get("/practicas/mis-materias");
        ...
        const resultados = await Promise.all(
          materias.map(async m => {
            const [parciales, inscritos, notasResumen] = await Promise.all([
              apiFetch.get(`/practicas/${m.id_materia}`) as Promise<Parcial[]>,
              apiFetch.get(`/practicas/${m.id_materia}/estudiantes`) as Promise<any[]>,
              apiFetch.get(`/practicas/${m.id_materia}/notas-resumen`) as Promise<
  ...
  return (
    <div className="dd-tab" style={{ maxWidth: 1000 }}>
      <header className="dd-tab-header">
        ...
        <p className="dd-tab-sub">Vista general de tus materias, estudiantes y próximas prácticas</p>
      </header>
          ...
          {data.map(({ materia, parciales, totalInscritos }) => (
            ...
            >
              ...
              <div className="dr-materia-meta">
                ...
                <span className="dr-meta-item">
                  <span className="dr-meta-icon">📅</span>
                  {parciales.length} práctica{parciales.length !== 1 ? "s" : ""}
                </span>
                <span className="dr-meta-item">
                  <span className="dr-meta-icon">👥</span>
      ...
      {/* ── Sección 3: Calendario de prácticas ──────────────────────────── */}
      <Section title="Calendario de prácticas" icon="🗓">
        <Calendario
          mes={calMes}
          anio={calAnio}
        ...
        <div className="dr-proximos">
          <div className="dr-proximos-title">Próximas prácticas</div>
          {(() => {
              ...
              return (
                <div key={p.id_parcial} className="dr-proximo-item">
                  ...
                  <div className="dr-proximo-info">
                    <div className="dr-proximo-nombre">{p.nombre_parcial ?? "Práctica"}</div>
        ...
        {celdas.map((dia, i) => {
          ...
          return (
            <div
              ...
              title={tieneParc
                ? parcialesPorDia[dia!].map(p => `${p.sigla}: ${p.nombre_parcial ?? "Práctica"}`).join("\n")
                : undefined
              }
            >
