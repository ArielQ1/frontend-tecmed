...
export function AuxiliarPracticas({ onVerNotas }: Props) {
  ...
  const cargarParciales = useCallback((m: Materia) => {
    setLoadingP(true);
    setParciales([]);
    apiFetch.get(`/practicas/${m.id_materia}`)
  ...
  return (
    <div className="dd-tab">
      <header className="dd-tab-header">
        <h1 className="dd-tab-title">Prácticas</h1>
        <p className="dd-tab-sub">Gestiona las prácticas de tus materias</p>
      </header>
                        ...
                        <div className="dd-row-actions">
                          <button
                            ...
                            onClick={() => onVerNotas({
                              ...
                              nombre_parcial: p.nombre_parcial ?? "Práctica",
                              sigla:          materia.sigla,
                              fecha:          p.fecha,
...
function ParcialModal({
  ...
  async function submit() {
    ...
    try {
      if (isEdit) {
        await apiFetch.put(`/practicas/${materia.id_materia}/${parcial!.id_parcial}`, body);
      } else {
        await apiFetch.post(`/practicas/${materia.id_materia}`, { ...body, tipo: "practica" });
      }
      onSaved();
    } catch (e: unknown) {
      ...
      <div className="dd-modal">
        ...
        <div className="dd-modal-actions">
          ...
          <button className="dd-submit-btn" onClick={submit} disabled={loading}>
            {loading ? "Guardando…" : isEdit ? "Guardar cambios" : "Crear práctica"}
          </button>
        </div>
      </div>
    </div>
...
function DeleteModal({
  ...
  async function eliminar() {
    setLoading(true);
    try {
      await apiFetch.delete(`/practicas/${materia.id_materia}/${parcial.id_parcial}`);
      onDeleted();
    } catch (e: unknown) {
  ...
  return (
    <div className="dd-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="dd-modal">
        <h2 className="dd-modal-title">Eliminar práctica</h2>
